...
def setspread(bot):
	...
	if bot.priceSpreadType == 1:
		...
		spreadmin = 0.1  #float(input('Type spread to start from in decimals like 0.05:  '))
		spreadmax = 1.0 # float(input('Type spread to end with in decimals like 0.55:  '))
		step = 0.05 #float(input('Type spread step'))
		for spread in np.arange(Decimal(spreadmin), Decimal(spreadmax), Decimal(step)):
			cfg = haasomeClient.customBotApi.setup_flash_crash_bot(bot.accountId, bot.guid, bot.name, bot.priceMarket.primaryCurrency, bot.priceMarket.secondaryCurrency, bot.currentFeePercentage, bot.basePrice, 1, bot.priceSpread, bot.percentageBoost, bot.minPercentage,bot.maxPercentage,bot.amountType, bot.amountSpread, bot.totalBuyAmount, bot.totalSellAmount, bot.refillDelay, bot.safetyEnabled, bot.safetyTriggerLevel, bot.safetyMoveInMarket, bot.safetyMoveOutMarket,bot.followTheTrend, bot.followTheTrendChannelRange, bot.followTheTrendChannelOffset, bot.followTheTrendTimeout)
	...
	if bot.priceSpreadType == 2: #percentage with boost
		...
		for boost in np.arange(0.2, 1.2, 0.1):
			...
			# spread = input('Type spread between orders in secondary currency : ')
				cfg = haasomeClient.customBotApi.setup_flash_crash_bot(bot.accountId, bot.guid, bot.name, bot.priceMarket.primaryCurrency, bot.priceMarket.secondaryCurrency, bot.currentFeePercentage, bot.basePrice, 3, bot.priceSpread, bot.percentageBoost, bot.minPercentage,bot.maxPercentage,bot.amountType, bot.amountSpread, bot.totalBuyAmount, bot.totalSellAmount, bot.refillDelay, bot.safetyEnabled, bot.safetyTriggerLevel, bot.safetyMoveInMarket, bot.safetyMoveOutMarket,bot.followTheTrend, bot.followTheTrendChannelRange, bot.followTheTrendChannelOffset, bot.followTheTrendTimeout)
				bt = btfcb(bot, haasomeClient, ticks)
				print(bt.roi)
				print('configuring bot parameters: ', cfg.errorCode, cfg.errorMessage)
		return cfg.result
	...
	if bot.priceSpreadType == 3: #exp
		print('bot price spread type is set to Fixed ammount.\n')
		spreadmin = 0.5  #float(input('Type spread to start from in decimals like 0.05:  '))
		spreadmax = 1.0 # float(input('Type spread to end with in decimals like 0.55:  '))
		step = 0.1 #float(input('Type spread step'))
		boost = 0.01
		multiplyer = 0.1
		for spread in np.arange(float(spreadmin), float(spreadmax), float(step)):
			cfg = haasomeClient.customBotApi.setup_flash_crash_bot(accountguid=bot.accountId, botguid=bot.guid, botname=bot.name, primarycoin=bot.priceMarket.primaryCurrency, secondarycoin=bot.priceMarket.secondaryCurrency, fee=bot.currentFeePercentage, baseprice=bot.basePrice, priceSpreadType=3, pricespread=spread, percentageboost= multiplyer, minpercentage=bot.minPercentage,maxpercentage=bot.maxPercentage,amounttype=bot.amountType, amountspread=bot.amountSpread, buyamount=bot.totalBuyAmount, sellamount=bot.totalSellAmount, refilldelay=bot.refillDelay, safetyenabled=bot.safetyEnabled, safetytriggerlevel=bot.safetyTriggerLevel, safetymovein=bot.safetyMoveInMarket, safetymoveout=bot.safetyMoveOutMarket,followthetrend=bot.followTheTrend, followthetrendchannelrange=bot.followTheTrendChannelRange, followthetrendchanneloffset=bot.followTheTrendChannelOffset, followthetrendtimeout=bot.followTheTrendTimeout)
	...
	if bot.priceSpreadType == 0: #exp
		...
		multiplyer = 0.1
		for spread in np.arange(float(spreadmin), float(spreadmax), float(step)):
			cfg = haasomeClient.customBotApi.setup_flash_crash_bot(accountguid=bot.accountId, botguid=bot.guid, botname=bot.name, primarycoin=bot.priceMarket.primaryCurrency, secondarycoin=bot.priceMarket.secondaryCurrency, fee=bot.currentFeePercentage, baseprice=bot.basePrice, priceSpreadType=0, pricespread=spread, percentageboost=bot.percentageBoost, minpercentage=bot.minPercentage,maxpercentage=bot.maxPercentage,amounttype=bot.amountType, amountspread=bot.amountSpread, buyamount=bot.totalBuyAmount, sellamount=bot.totalSellAmount, refilldelay=bot.refillDelay, safetyenabled=bot.safetyEnabled, safetytriggerlevel=bot.safetyTriggerLevel, safetymovein=bot.safetyMoveInMarket, safetymoveout=bot.safetyMoveOutMarket,followthetrend=bot.followTheTrend, followthetrendchannelrange=bot.followTheTrendChannelRange, followthetrendchanneloffset=bot.followTheTrendChannelOffset, followthetrendtimeout=bot.followTheTrendTimeout)
			bt = btfcb(bot, haasomeClient, ticks)
