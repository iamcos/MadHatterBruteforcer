...
def btindiator(bot, indicator_guid, haasomeClient):
    ...
    change_param = haasomeClient.tradeBotApi.edit_bot_indicator_settings(
        bot.guid, indicator_guid, field_n, value
    )
    ...
    if gettradebot.indicators[guid].indicatorTypeShortName == "CRSI":  # missing ROC
        for l in np.arange(2, 40, 2):
            ...
            for b in np.arange(10, 40, 2):
                change = haasomeClient.tradeBotApi.edit_bot_indicator_settings(
                    gettradebot.guid, guid, 3, b
                )
                bt = haasomeClient.tradeBotApi.backtest_trade_bot(
                    gettradebot.guid, ticks
                )
                printerrors(bt, "bt")
                printerrors(change, "change")
                print(bt.result.roi)
...
def modify_indicator(bot, haasomeClient, indicator_guid, field, value):
    ...
    modifyparam = haasomeClient.tradeBotApi.edit_bot_indicator_settings(
        bot_guid=bot.guid, elementguid=indicator_guid, field=field, value=value
    )
...
def get_options(indicator_guid, bot):
    ...
    for option in bot.indicators[str(indicator_guid)].indicatorInterface:
        indicator_options.append(
            [
                bot.indicators[indicator_guid].indicatorTypeShortName,
                option.title,
                option.value,
                indicator_guid,
                bot.guid,
...
def indicatorstodict2(botty):
    ...
    for indicator in bot.indicators:
        ...
        if bot.indicators[indicator].indicatorTypeShortName in sections:
            ...
            indicator_options[
                bot.indicators[indicator].indicatorTypeShortName
            ] = params2
