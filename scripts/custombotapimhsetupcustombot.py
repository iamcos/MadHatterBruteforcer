def setup_mad_hatter_bot(
    ...
    position: str,
    fee: float,
    tradeAmountType: EnumBotTradeAmount,
    tradeAmount: float,
    useconsensus: bool,
    disableAfterStopLoss: bool,
    interval: int,
    includeIncompleteInterval: bool,
    ...
    try:
        return HaasomeClientResponse(
            ...
            self._convert_json_bot_to_custom_bot_specific(
                EnumCustomBotType.MAD_HATTER_BOT, response["Result"]
            ),
        )
    except:
        return HaasomeClientResponse(
            EnumErrorCode(int(response["ErrorCode"])), response["ErrorMessage"], {}
        )
