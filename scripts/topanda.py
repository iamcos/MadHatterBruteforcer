...
class BotReader(ApiBase):
    ...
    def _convert_json_bot_to_trade_bot_object(self, jsonstr: str):
        ...
        for k, v in botinitial.indicators.items():
            ...
            for indicatoroption in indicator.indicatorInterface:
                indicatoroptions.append(
                    super()._from_json(indicatoroption, IndicatorOption)
                )
        ...
        for k, v in botinitial.insurances.items():
            ...
            for insuranceoption in insurance.insuranceInterface:
                insuranceoptions.append(
                    super()._from_json(insuranceoption, IndicatorOption)
                )
    ...
    def _convert_json_bot_to_custom_bot_specific(
        self, bottype: EnumCustomBotType, jsonstr: str
    ):
    ...
    def get_custom_bot(filename):
        ...
        botdata = _convert_json_bot_to_custom_bot_specific(
            EnumCustomBotType.MAD_HATTER_BOT, response["Result"]
        )
        print(botdata)
        ...
        if type(data) is dict:
            ...
            if issubclass(cls, List):
                ...
                for key, value in data.items():
                    instance.update(
                        {
                            ApiBase._from_json(key, key_type): ApiBase._from_json(
                                value, val_type
                            )
                        }
                    )
