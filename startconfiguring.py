...
def selectparametertochange(guid):
...
def startconfiguring(therange, guid, btinterval):
	while True:
		answers = prompt(selectparametertochange(guid))
		if answers['selectedparameter'] == 'interval':
				answers = prompt(savehistory.makepairmenu(guid))
				print(answers)
		...
		if answers['selectedparameter'] == 'Length':
			tuner.tuneLength(therange, guid, btinterval)
		if answers['selectedparameter'] == 'Devup':
			tuner.tuneDevup(therange, guid, btinterval)
		if answers['selectedparameter'] == 'Devdn':
			tuner.tuneDevdn(therange, guid, btinterval)
		if answers['selectedparameter'] == 'RsiLength':
				tuner.tuneRsiLength(therange, guid, btinterval)
		if answers['selectedparameter'] == 'RsiOversold':
				tuner.tuneRsiOversold(therange, guid, btinterval)
		if answers['selectedparameter'] == 'RsiOverbought':
				tuner.tuneRsiOverbought(therange, guid, btinterval)
		if answers['selectedparameter'] == 'MacdFast':
			tuner.tuneMacdFast(therange, guid, btinterval)
		if answers['selectedparameter'] == 'MacdSlow':
			tuner.tuneMacdSlow(therange, guid, btinterval)
		if answers['selectedparameter'] == 'MacdSign':
			tuner.tuneMacdSignal(therange, guid, btinterval)
