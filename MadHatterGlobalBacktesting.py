...
def backtest_mh_indicator_ranges(bot):
	...
	results = []
	filename = str(bot.priceMarket.primaryCurrency)+'\\'+str(bot.priceMarket.secondaryCurrency)+' '+str(ticks)+'.csv'
	with open('filename', 'w', newline='') as csvfile:
		currentfile = Path(str(filename))
		currentfile.touch(exist_ok=True)
		print(filename, 'created!')
		fieldnames = ['ROI','signalconsensus','resetmiddle', 'allowmidsells', 'matype', 'rsil', 'rsib', 'rsis', 'bbl', 'devup', 'devdn', 'macdfast', 'macdslow', 'macdsign']
		csvwriter = csv.DictWriter(csvfile,fieldnames=fieldnames)
		csvwriter.writeheader()
		for signalconsensus in [False, True]:
										...
										bot.guid, EnumMadHatterIndicators.BBANDS, 1, devup)
											for devdn in np.arange(0.8,2.1,0.2):
												...
												for macdfast in range(10,50,5):
													...
													for macdslow in range(20,100,5):
														if macdslow<=macdfast:
															...
															for macdsign in range(0,21,2):
																if macdsign==macdslow and macdsign==macdfast:
																	pass
																else:
																	bot1=haasomeClient.customBotApi.set_mad_hatter_indicator_parameter(
																	...
																	with open('filename', 'w', newline='') as csvfile:
																		fieldnames = ['ROI','signalconsensus','resetmiddle', 'allowmidsells', 'matype', 'rsil', 'rsib', 'rsis', 'bbl', 'devup', 'devdn', 'macdfast', 'macdslow', 'macdsign']
																		for v in(results)
																			csvwriter.writerow({'ROI':str(bt.result.roi),'signalconsensus':str(signalconsensus),'resetmiddle': str(resetmiddle), 'allowmidsells': str(allowmidsells), 'matype': str(matype), 'rsil': str(rsil), 'rsib': str(rsib), 'rsis': str(rsis), 'bbl': str(bbl), 'devup': str(devup), 'devdn': str(devdn), 'macdfast': str(macdfast), 'macdslow': str(macdslow), 'macdsign': str(macdsign)})
